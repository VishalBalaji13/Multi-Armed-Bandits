...
class KArmedBanditEnv(BaseEnv):
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[Dict[str, Any]] = None) -> Tuple[int, Dict[str, Any]]:
        ...
        #For each arm the sample stationary probabilities
        self.p = self._rng.uniform(0.0, 1.0, size=self.k)
        obs = 0
        info = {"p": self.p.copy(), "optimal": 0}
        return obs, info
    ...
    def step(self, action: int) -> Tuple[int, int, bool, bool, Dict[str, Any]]:
        ...
        #Applying non-stationary drift
        if self.nonstationary:
            noise = self._rng.normal(0.0, self.sigma, size=self.k)
            self.p = np.clip(self.p + noise, 0.0, 1.0)
        #Selected arm's Bernoulli reward
        prob = float(self.p[action])
        reward = int(self._rng.random() < prob)
        #Optimal action indicator under current probabilities
        optimal_action = int(np.argmax(self.p))
        optimal = int(action == optimal_action)
            
        obs = 0
        terminated = False
        truncated = False
        info = {"p": self.p.copy(), "optimal": optimal}
